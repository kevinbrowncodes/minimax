...
- **A fix is verified on the shipped artefact's own output, never on a script that re-implements it.** BUG_006 added a three-second rule to the shot-change node and was marked Resolved from a scratchpad script's numbers; the node itself emitted one value per job for two days (BUG_010, found when the node's real output was finally read from ComfyUI's history). A re-implementation proves the idea; only the artefact's output proves the fix — and if nothing in the gate runs the artefact (a Python node in a TypeScript gate), the fix ships with the first test that does.
