...
import { isAgentDraws, SETTING_KEYS, SETTING_TYPES, type Settings } from "@/lib/settings";
...
export function PATCH(request: Request): Promise<Response> {
  return guarded(async () => {
    ...
    for (const [key, value] of entries) {
      ...
      const wanted: "boolean" | "string" | readonly string[] | readonly number[] = SETTING_TYPES[key as keyof typeof SETTING_TYPES];
      const allowed = typeof wanted === "string" ? typeof value === wanted : (wanted as readonly unknown[]).includes(value);
