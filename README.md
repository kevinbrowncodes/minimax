...
| **Scheduled** — the queue of generations (the reference's page is its agent's timed tasks; ours is the owner's meaning): Send never fails as busy — beyond the adapter's five open jobs the request waits in the app's own queue (`/data/queue.json`, its images with it) as a history entry whose task page reads "Waiting — Nth in line", and the runner (inside the polled routes — no process of its own) submits the next due request as slots free, in order. The page lists Running (with Stop), Waiting (in order: ↑ ↓, **Run at…** a time, **Edit** — the composer prefilled, Send replaces the entry in place —, Remove), Done today and Failed, with a search and a status filter; Create and a **Run at…** beside Send on the composer | STORY_041 |
