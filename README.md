...
| **Management** (the Plugins row): **Plugins** shows the one plugin, video-creator, described from `GET /api/capabilities` (model, resolution, durations, reachable or not) with its Details and a switch (`PATCH /api/settings { videoEnabled }`) that makes a text-only workstation when off (no Video generation chip, no video controls) and restores it when on; **Skills** (STORY_054) has two sections: **Director skills** — the folders under `agents/skills/` the agent can follow (`GET /api/agent/skills`), each with a *Folder* badge, its description and a meta line from its metadata (model · checkpoint · ComfyUI · adapter · verified date), and **Use**, which opens the composer in video mode with the Agent chip on and that skill chosen (`/?agent=<id>`, the choice saved as `agentSkill`); and **Templates** — STORY_040's prompt recipes (`/data/skills.json`; a built-in Short-to-script whose template is the STORY_020 base prompt scaffold) with search, Create / Edit / Delete and **Use**. The search runs over both sections. Under + › Skills the directors are radio rows (picking one turns the chip on; greyed with the reason in text or extend mode), then **Templates ›** where a template drops its text into the composer with `{{idea}}` replaced by what was typed, then Manage skills and Add template; **Apps** says plainly there are none; the Agents tab stays the read-only editor (STORY_038 deferred) | STORY_040, STORY_054 |
| **Scheduled** — the queue of generations (the reference's page is its agent's timed tasks; ours is the owner's meaning): Send never fails as busy — beyond the adapter's five open jobs the request waits in the app's own queue (`/data/queue.json`, its images with it) as a history entry whose task page reads "Waiting — Nth in line", and the runner submits the next due request as slots free, in order — from the polled routes and, since STORY_042, from a 30 s ticker in the app server's start-up hook (`QUEUE_TICK_MS`, 0 disables), so a run-at goes with no browser open. The page lists Running (with Stop), Waiting (in order: ↑ ↓, **Run at…** a time, **Edit** — the composer prefilled, Send replaces the entry in place —, Remove), Done today and Failed, with a search and a status filter; Create and a **Run at…** beside Send on the composer. Since STORY_043 an extension of a clip that is still queued or running waits in the line ("Waiting · after <stamp>") and goes the moment its source is done, with the source's real job id (BUG_007) — browser or no browser: since [BUG_009](docs/bug/BUG_009_a_queued_extension_waits_for_its_source_only_while_a_browser_is_watching_it.md) the runner itself asks the adapter about every source the line waits on, once per tick for a source nobody has heard from in `QUEUE_SOURCE_STALE_MS` (default 10 s; a page watching the job costs no extra request), and records the answer as a page's poll would; **Queue an extension** on the Running and Waiting rows and Extend on a pending task page (the tile shows the requested length) — and since STORY_044 a chain is **one Send**: paste the scene and every script into the composer (each script starting with its `[0:00-` line), attach the image, and the strip under the box lists the segments, the length in all and the Overlap; **Send all** queues them as a chain and opens the last segment's page, where the whole video lands; segments the Spark would refuse (a source over its 30 s cap) are greyed and Send all waits until they fit | STORY_041, STORY_043, STORY_044 |
...
**The prompt.** Since 2026-09-14 the adapter builds MiniMax's documented prompt format around what you type (its [base prompt guide](docs/references/prompt-guides/VIDEO_PROMPT_WRITING_GUIDE_base_en.txt), read not recalled): for a first frame, the instruction line `For the target video, at 0.00 seconds into the target video, <Picture 1> (from [Shot 1]) is fully referenced.`; then `[Shot 1] Live-action. The camera holds a perfectly static shot throughout the entire S.SS-second duration: no cut, no dissolve, no transition and no change of framing; …` followed by your text as one paragraph (a line's leading `[0:03-0:07]` becomes `From 0:03 to 0:07,` — MiniMax's only timestamps mark cuts), then the soundscape and music fields. An extension gets no instruction line (its preserved head is not a picture). Text that already starts with `integrated_multimodal_description:` is sent as is. The prompt limit is 6,000 characters, room for MiniMax's own 350–700-word style. What the adapter cannot do is describe your scene: write it yourself at the top of the script — [docs/scripts/scene.txt](docs/scripts/scene.txt) is the paragraph for `01.jpg` — because the model's own examples spend most of their words on the set, the light and the subject, and a script that names only the action leaves the model to invent the rest ([STORY_020](docs/story/STORY_020_a_video_stays_in_one_shot_to_the_end_and_a_cut_the_model_makes_anyway_is_flagged_before_the_owner_sees_it.md) has the evidence). `docker logs minimax-adapter` prints every prompt as sent. To have an LLM write the whole prompt in MiniMax's format from one image — the scene, the camera, a single-shot timeline and the sound fields — use the skill in [agents/skills/minimax-h3-director-thirst-trap/](agents/skills/minimax-h3-director-thirst-trap/SKILL.md); its output starts with the instruction line, so the adapter sends it unchanged, and its `metadata` names the model, checkpoint and versions it was written against.

**The shot-change check.** Every finished clip is measured in the graph (our node `MiniMaxLocalFrameChanges`, [spark/comfyui/custom_nodes/minimax_local/](spark/comfyui/custom_nodes/minimax_local/)): the picture's outer border — the set, not the person — compared one second apart and three seconds apart. A border that moves by 30 or more (mean absolute RGB) over one second, or by 20 or more over three seconds (BUG_006: a slow dissolve of the set), is a shot change: a cut, a dissolve or a camera move; a held shot never moved it more than 11 over a second or 16 over three on the clips measured, and a squat moved the whole picture by 66 but the border by 16. The job's `result.cuts` lists where (`frame`, `seconds`, `kind`; `[]` when the shot held), the adapter log says so, and the task page shows "The shot changed at 00:11 …" above the result card with a Retry that draws a new seed (mounted by CHORE_009 with STORY_023). Calibrated for static-camera prompts: a prompt that moves the camera trips those two rules by design — so since 2026-09-16 ([STORY_046](docs/story/STORY_046_the_shot_change_notice_tells_a_camera_move_the_prompt_asked_for_from_a_cut.md)) a third rule tells the two apart: a border that jumps by 30 or more between **two consecutive frames** is a `cut` (the real cuts on disk step 46.5–49.7 in one frame; a handheld selfie at most 15.7), anything else is `framing`, and the adapter reads what the prompt asked of the camera into `result.camera` (`static` / `moving` / `unknown`, from the guide's move vocabulary — push in, pan, tilt, tracking shot, zoom, handheld, sway …). The amber notice with Retry is for a cut on any camera, or framing on a prompt that did not ask for a moving camera; framing alone on a moving camera is a quiet line under the result — "The framing moved at 00:01, 00:04 and 00:08, as the prompt asked; no cut." — with no Retry and no Inbox event. Contract v1.4. The three-second rule measured only the first three seconds of every clip from 2026-09-14 to 2026-09-16 ([BUG_010](docs/bug/BUG_010_the_three_second_shot_change_rule_measures_only_the_first_three_seconds.md), fixed; the node now has a unit test, `spark/comfyui/test-nodes.sh`, run inside the ComfyUI image).

### Agent mode — a director in the cloud (EPIC_009; STORY_047 the credential)

**Status (2026-09-18 12:00 EDT): STORY_047–056 Done — every story of the epic: the credential, the spike, the director on the server, the Agent chip, Confirm before generating, Agent instructions, the chain director, the Skills tab, draws per prompt, and Retry chain (the tenth, from the night's second chain).** Attach a photo, turn **Agent** on, type notes or nothing, Send: the director's prompt comes back into the box for review with the Duration set to the skill's 10 s and "≈ 50 min on the Spark" under Send; Send again and it is a job. A refusal or an error shows where the prompt would be and as an Inbox row under Messages; nothing is queued. The model is **`gemini-3.8-flash`** (GA, read from Vertex with the key on 2026-09-17 — the newest Flash), served from the **`global`** location (`us-central1` answers 404 for it on this project), pinned in `lib/agent-config.ts` and overridable by `VERTEX_MODEL`. The one step still done by hand — writing the prompt from the photo — moves to a **Gemini Flash** model on the owner's own Google Cloud project through **Vertex AI**, following the director skill under `agents/skills/`. **The photos and the notes given to the agent go to Google.** Everything else stays on the Spark, and the gate never calls Google (STORY_049's fake stands in). The stories: [EPIC_009](docs/epic/EPIC_009_agent_mode_a_director_writes_the_prompt_from_the_photo.md).

| Item | Value |
| --- | --- |
| The credential | A service account (`minimax-agent`, `roles/aiplatform.user` only) on a GCP project of the owner's — an existing one renamed **"MiniMax H3 Agent"** (2026-09-17: the account's project quota is 6, a fresh one waits for the increase he requested; the project's id lives in `.env`, not here) with a **$50/month budget** that emails him at 50 / 90 / 100 % (Google has no hard cap); its key at `spark/data/secrets/vertex-sa.json` (gitignored under `spark/data/`, mode 600, the owner's uid), mounted **read-only** into the `app` container at `/secrets/vertex-sa.json`. Never in git, never printed, never pasted — presence is tested (`spark/gcloud/status.sh` › "key written: yes") |
| The one-time setup | From Google's official CLI image (`google-cloud-cli:585.0.0-slim`, pinned by digest 2026-09-17), the `gcloud` service of `compose.yaml` (profile `tools`), nothing installed on the box: **the owner runs `spark/gcloud/login.sh` in his own terminal** — gcloud prints a URL, he approves it in his browser and pastes the code back (the login persists under `spark/data/gcloud/`, gitignored; a code never passes through a chat); then `spark/gcloud/setup-vertex.sh <project-id> <billing-account-id> [region]` creates the project, links billing, enables `aiplatform.googleapis.com`, checks the `iam.disableServiceAccountKeyCreation` policy first (if an organisation enforces it: `login.sh --adc` writes the owner's application-default credentials to the same path instead), creates the service account, writes the key and fills `.env`. Idempotent; `status.sh` says where things stand |
| Env vars | `VERTEX_PROJECT` and `VERTEX_LOCATION` (default `global`) — written by `setup-vertex.sh` into `.env`; `VERTEX_MODEL` — overrides the pinned `gemini-3.8-flash`; `GOOGLE_APPLICATION_CREDENTIALS` — the key's in-container path, set by compose. All three `VERTEX_*` can be overridden from Settings › Environment variables (the store wins over `.env`). See `.env.example` |
| What the spike found (STORY_048, 17 calls, every reply in [the replies file](docs/story/STORY_048_the_gemini_spike_replies.md)) | **No refusal** on any of the three photos at the default thresholds or with `OFF` on all five text harm categories (accepted without an allowlist — the app sends OFF). The format is right every time (the instruction line, `[Shot 1]`, the three fields, no timestamps, no fences). The model writes **≈ 250-word descriptions whatever the length rule says**; a **second pass** asking it to expand its own draft gives 418–491 words with the anchor the skill wants — the shape the route uses. Thinking adds ≈ 6k tokens and no length → `thinkingLevel: LOW`; the output cap is 16,384 (a 4,096 cap is eaten by thinking). `mediaResolution` LOW would save 9 % of the prompt; not used. Latency is Google's: 7–107 s per pass, median ≈ 15 s |
| Price (read from `cloud.google.com/vertex-ai/generative-ai/pricing` on 2026-09-17) | Gemini 3.8 Flash, global, introductory through 2026-12-31: **$0.75 / 1M input, $3.75 / 1M output incl. thinking, $0.075 / 1M cached input**; from 2027-01-01 $1.50 / $7.50 / $0.15. A run (two passes, thinking LOW, ≈ 9.5k + 10.1k tokens in, ≈ 1k out) is **≈ 1.9 ¢** today, ≈ 3.8 ¢ from 2027. **Context caching**: the skill + references are 8,170 tokens; a cache saves 0.55 ¢ a call and costs 0.82 ¢ an hour to hold — break-even ≈ 1.5 calls/hour sustained, so not used. The project carries a $50/month budget with email alerts |
| Running the spike | `spark/gcloud/spike.sh <skill-dir> <image> [notes…] [--safety default|least] [--expand] [--thinking low] [--max-tokens N] [--media low|medium|high]`, or `--models` — the `agent-spike` compose service (the gate image with the key mounted read-only; the `gate` service itself never sees the key). Replies land under `spark/data/agent-spike/` (gitignored). **Every photo and note given to it goes to Google.** |
| The routes (STORY_049) | `GET /api/agent/skills` — the folders under `SKILLS_DIR` (baked into the image at `/srv/agents/skills`; the gate points at the repo's) with each `SKILL.md`'s name, description and metadata. `POST /api/agent/runs` (multipart: `skill`, one `referenceImage`, `notes`) — the skill's body as the system instruction, its references, the photo and the notes in a fixed order, **two passes** (the draft, then the model expands its own draft to 450–600 words), `OFF` on the five text harm categories, `maxOutputTokens` 16,384, the thinking level from **`VERTEX_THINKING`** (`low` default; `medium`, `high`, `default`), and the **format check** (`lib/prompt-format.ts`: the instruction line, `[Shot 1]`, the three fields, 350–600 words, no timestamps, no markdown — per segment for a chain) → `{ kind: "prompt", prompt, findings, segments, passes }` with the findings beside the prompt (a warning in review, a block only straight-through), or `{ kind: "refusal", message }` in the model's words (a stop for safety, or prose instead of a prompt), or an error naming the cause (`not_configured` 503, `unknown_skill` 400, `quota` 429, `unreachable` 502, `timeout` 504 after `AGENT_TIMEOUT_MS`, 240 s). Aborting the request aborts the call to Google. A run that ends without a prompt — a refusal or an error, never a Stop — is kept in `agent-runs.json` beside the history (`GET /api/agent/runs`, `PATCH /api/agent/runs/:id { openedAt }`) for the Inbox (STORY_050) |
| The gate | never calls Google: the stub's **fake Vertex** answers the token path, `publishers.models.get` and `generateContent` by script name (`clean`, `warn`, `chain`, `chain-warn`, `refusal`, `refusal-text`, `malformed`, `slow`, `quota`) and records what it was sent (`/__stub/agent/runs`); `playwright.config.ts` and the integration lane point `VERTEX_BASE_URL` / `VERTEX_TOKEN_URL` at it and generate a throw-away key pair per run — no key is ever committed (a guard test scans for one) |
| The chip (STORY_050) | In video mode, beside `+`, where the reference keeps its Agent Team switch: off — *Agent*, muted; on — *Agent · Thirst trap* with a ⌄ that opens the Skills menu (the folders, the chosen one saved as the `agentSkill` setting; *Manage skills* → Management › Skills) and the right-hand pill reading the agent's model (*Gemini 3.8 Flash*, one row). Send needs one photo; notes are optional. While the director works the box is read-only, *Thinking…* shows and Send is **Stop**. The reply fills the box, the chip turns off, the photo and ratio stay, the Duration becomes the skill's `minimax-clip-seconds`; findings show in an amber strip and never block. A refusal is the alert in the model's words with the chip still on; it is also an Inbox row under **Messages** that reopens the composer with the notes. Greyed with the reason when the agent is not configured, and in extend mode ("Agent needs a photo"). Both widths, both themes; the e2e lane picks the fake's outcome with `?agentScript=` |
| Confirm before generating (STORY_051) | The ⚙ beside the chip opens **Agent settings** (Flow's panel in the Settings dialog's chrome): *Always* — the reply comes back for review (the default); *Never* — a clean single prompt goes straight to a job with the composer's parameters and the skill's clip length, the toast *Queued — the director's prompt* and the task page, no review step. A reply with findings is not sent under *Never* (the strip says so; edit and Send), a clean chain is queued whole (STORY_053), a refusal is never a job. The setting is `agentConfirm` in `settings.json` |
| Agent instructions (STORY_052) | The ≡ beside the chip opens **Agent instructions** (Flow's panel): rows of a switch, a title, a guideline and an optional reference image — picked from Assets › From you (a job's own reference image) or uploaded — that go with **every** run after the skill's references and before the photo, each as *Instruction — title* with its image labelled for it. A saved scene (the set's photo + its anchor paragraph) or a character is reused with no one at the keyboard; the ≡ shows how many are active and the status line says *Thinking… (N instructions)*. Stored in `agent-instructions.json` beside the history; uploads under `uploads/agent-instructions/` |
| The chain director (STORY_053) | The second skill, `agents/skills/minimax-h3-director-thirst-trap-chain/` (*Chain director* in the menu): one continuous story from the photo in N ten-second segments (the notes say how many — "4 segments"; the default 3) — one scene anchor with no pose in it written into every segment, segment 1 in the single-clip format with the instruction line, segments 2..N starting at `integrated_multimodal_description:` with no picture, written for the extension's real 12.25 s whose first 1.6 s are the previous hold. The composer's splitter learns the format (`lib/chain.ts`: no `[0:00-` line and two or more markers → a segment per marker, nothing added to any), so the reply comes back as STORY_044's strip — rows titled by each segment's first action sentence — and **Send all** queues the segments unchanged, each an extension of the one before. The format check runs per segment (segments 2..N under the extension rules: `instruction-line-on-extension` if a line or a `<Picture>` appears; every finding says *Segment N*), and it now also confirms the instruction line is the model's word for word (`instruction-line-differs`). **All clean or nothing queued** under *Never*: every segment clean → the three POSTs with no click, the toast *Queued — 3 segments, ≈ 31.4 s*, the last segment's page; one finding on any segment → *Not sent — Segment 2 misses the skill's format: …*, nothing posted. The adapter is untouched: segment 1 passes through with its line, the extensions without one |
| Draws per prompt (STORY_055) | Agent settings › Video generation default › **Draws x1 · x2 · x3 · x4** (`agentDraws` in `settings.json`, 1 by default; `PATCH /api/settings` takes 1–4). Every single-clip Send in video mode honours it — a hand-typed prompt, a director's reply after review, a straight-through reply, an extension — posting the same request that many times one after another (`lib/submit-job.ts › submitDraws`: the first with the run-at and, for an Edit, `replaces`; **never a seed** — the adapter draws one per job, so N requests are N draws); the toast reads *Queued — N draws* and the first draw's task page opens; the Spark's adapter takes one job at a time, so the rest wait in the line. Send shows **×N** and the line under it reads *≈ N × M min on the Spark*. A chain is sent once whatever the count (the strip says so). A refusal mid-way keeps the accepted draws and names the one refused; the composer keeps the prompt |
| Retry chain (STORY_056) | On a chain segment whose join cut (the shot-change notice) — or one that failed — with segments queued behind it, **Retry** reads *Retry chain*: `POST /api/jobs/:id/retry-chain` redraws the segment (the same request, no seed, an extension of its own source; a fresh clip with its reference images read back from disk) and re-queues every segment that continued from it, in order, each as an extension of the id just answered; a later segment still waiting or running is cancelled first, one that is done is left. `GET /api/history/:id` answers `chainAfter` (what continues from an entry, transitively). From the night of 09-17/18: two chains, six joins, one cut — Retry alone left segment 3 on the bad draw |
| What the app shows | `GET /api/capabilities` › `agent: { configured, reason?, model? }` — the app's own field merged over the adapter's JSON (the adapter is untouched). With the key mounted it reads `configured: true` (verified live 2026-09-17); without one, the reason — STORY_050's Agent chip greys out with it |
