...
| What the app shows | `GET /api/capabilities` › `agent: { configured, reason? }` — the app's own field merged over the adapter's JSON (the adapter is untouched). With the key mounted it reads `configured: true` (verified live 2026-09-17); without one, the reason — STORY_050's Agent chip greys out with it |
