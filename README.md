...
| Env vars | `VERTEX_PROJECT` and `VERTEX_LOCATION` (default `us-central1`) — written by `setup-vertex.sh` into `.env`; `VERTEX_MODEL` — the Gemini model id, **unset until STORY_048's spike pins it from Vertex's model list**; `GOOGLE_APPLICATION_CREDENTIALS` — the key's in-container path, set by compose. All three `VERTEX_*` can be overridden from Settings › Environment variables (the store wins over `.env`). See `.env.example` |
| What the app shows | `GET /api/capabilities` › `agent: { configured, reason? }` — the app's own field merged over the adapter's JSON (the adapter is untouched). Until the model is pinned it reads `configured: false, reason: "VERTEX_MODEL is not set — STORY_048's spike pins it…"`, and STORY_050's Agent chip will grey out with that reason |
