...
**Status:** Open (2026-09-14) — every remedy measured or withdrawn by 2026-09-19 (STORY_060); closes when STORY_061 ships · **Priority:** High — it is the one way STORY_017's result can still fail the owner's definition of extending
...
2. ~~**Overlap 56 frames (2.3 s)** — more real footage in the clip's own timeline; one click in the UI.~~ *(STORY_060, 2026-09-18: measured on the segment-2 prompt over `eb90ccb2` with the three known seeds — 0 of 3 held at overlap 56 against 1 of 3 at 39; it broke the seed that held, and costs 72.8 min a draw against 66.2. Withdrawn as a remedy.)*
3. ~~**A keyframe at the end** (`MiniMaxH3AddGuide`, the source's last frame re-anchored at the segment's last frame) so the shot must return to the same framing — cheap in the graph, but it constrains the end pose.~~ *(STORY_060, 2026-09-18/19: the one lever that held — 8 of 8 anchored draws of segment 2 across five seeds, including the two that cut under everything else; the ends are guaranteed, not the middle — one chain segment cut away and back between its pinned ends. Promoted to [STORY_061](../story/STORY_061_the_segment_ends_on_its_first_frame.md), paired with the probe of [BACKLOG_012](BACKLOG_012_a_cheap_probe_tells_a_cutting_seed_before_the_hour_is_spent.md).)*
