...
- **The pill and the Skills tab — under discussion:** the three ways agents and skills can map onto the UI, with before/after mockups and a recommendation, are in [BACKLOG_009_agents_and_skills_a_map_with_mockups.md](BACKLOG_009_agents_and_skills_a_map_with_mockups.md); the owner decides there; after seeing Google Flow's composer (an **Agent** chip beside `+` that toggles agent mode on the same box) he wants that chip, and the map's Option D — Flow's chip in MiniMax's chrome: the skill on the chip, the model on the reference's right-hand pill, the folders on the Skills tab — is the recommendation. His first framing, kept for the record:
...
- ~~**Straight-through is a Settings switch, default off**~~ **Revised 13:45 after reading Google Flow's agent mode (the map, § 7): it is "Confirm before generating" — Always / Never — in an Agent settings panel opened from the chip, in Flow's words; Always (review in the composer) is the default.** Also taken from Flow: **Agent instructions** — persistent, toggleable guidelines with an optional reference image, sent after the skill on every run, doubling as saved scenes and characters; **draws per prompt (x1–x4)** as a later story; Send becoming Stop with a live status while the agent works; a "≈ N min on the Spark" line under Send. Left: image generation, Flow's placeholder and Expand button.
...
- **The Agent chip lives in video mode only** (13:55): agent mode is a modifier of the video composer — photo + skill → prompt → job. The reference's text-mode chat stays as it is; BACKLOG_006 decides its future (which could also be Gemini later).
- **An agent run that ends without a job** (a refusal, a failed format check, Stop) **shows in the composer** where the prompt would be, with the notes kept for a rephrase, **and is also an Inbox notice**, so a refusal from a run the owner walked away from is not lost. Nothing in history or Assets — no job existed.
- **The GCP side is set up by the assistant from a session the owner signs into once** — the recon browser's rule applied to `gcloud`: `gcloud` in a container on the Spark with a persistent, gitignored config volume; the owner runs the one-time `gcloud auth login` in his own browser (a URL he approves in Google — no password or token reaches the chat); from then on the assistant runs the steps with that session — enable the Vertex AI API, create the service account and its roles, write its key straight into the gitignored path the app container reads. The key never appears in the transcript; nothing is installed on the box. (The owner chose "you give me access and I set it up"; this is that, in the form CLAUDE.md § 4b allows.)
