...
| **Estimated completion** | ~~If approved this evening and the Spark stays free: STORY_044 verified ≈ 21:40 on 2026-09-15, STORY_045's minute done ≈ 19:15 EDT on 2026-09-16~~ — **refreshed 18:05 EDT, 2026-09-15:** the GPU is reserved until ≈ 05:00 EDT on 2026-09-16 for the owner's own overnight run (session `minimax-5b`: two 30 s chains from his `test/` folder, six generations through the queue — STORY_043's flow in earnest), during which no container of ours is restarted and nothing is deployed. So: **tonight, build and gate only** — BUG_008 and STORY_044 landed on `develop` if approved, undeployed; **2026-09-16 from ≈ 08:00**: STORY_044 deployed ≈ 08:15 and its 3 × 5 s chain done ≈ 09:25; STORY_045 built ≈ 11:45, the adapter rebuilt between jobs ≈ 12:00, the 31 s clip extended ≈ 13:15, the six-script minute started ≈ 13:30 and finished **≈ 20:00 EDT, 2026-09-16** — the epic's close. If nothing is approved tonight, add the build time to the morning: the close moves to ≈ 22:00. Every hour the overnight run overruns 05:00 moves the close by an hour |
| **Actual completion** | — |
...
| --- | --- | --- | --- | --- | --- | --- |
| — | [BUG_008 — Editing a waiting extension turns it into a fresh clip](../bug/BUG_008_editing_a_waiting_extension_turns_it_into_a_fresh_clip.md): `InitialRequest` carries `continueFrom` / `overlapFrames`, the composer reopens in extend mode, `replaces` keeps the source and history in step. First, so that Edit on a chain's segment is safe | Open | ≈ 30 min | — | 2026-09-15 ≈ 18:50 EDT landed if approved by 18:15 (deployed 2026-09-16 morning, after the overnight run) | — |
| 044 | [One starting frame and any number of scripts go out in one Send](../story/STORY_044_one_starting_frame_and_any_number_of_scripts_go_out_in_one_send.md): the text splits at every `[0:00-` line, the scene before the first goes with every segment, the strip under the box shows the segments and the length in all, **Send all** posts them one after another (each an extension of the one just answered — STORY_043 queues them), the last segment's task page is the whole video's; the cap read from the capabilities greys what would not run; titles from the first timestamped line. Works to three 10 s segments today, to six once 045 lands | Proposed | ≈ 90 min + ≈ 65 min GPU (+ ≈ 3 h, the owner's chain, when the GPU is his to give) | — | 2026-09-15 ≈ 20:30 EDT landed if approved tonight; 2026-09-16 ≈ 08:15 deployed, ≈ 09:25 verified | — |
| 045 | [A video can be extended past 30 seconds because the join happens outside the graph](../story/STORY_045_a_video_can_be_extended_past_30_seconds_because_the_join_happens_outside_the_graph.md): the adapter cuts the source's tail with ffmpeg and uploads only that; the graph saves only the new part; the adapter joins source + segment with ffmpeg into its own volume; the seam still measured (offset); `maxSourceSeconds` 30 → 120; the memory split re-derived from the measurement | Proposed | ≈ 2 h 20 min + ≈ 75 min GPU + ≈ 6 h 25 min the minute | — | 2026-09-16 ≈ 11:45 EDT built, ≈ 12:00 the adapter rebuilt, ≈ 13:15 the 31 s clip extended; ≈ 20:00 the minute done | — |
