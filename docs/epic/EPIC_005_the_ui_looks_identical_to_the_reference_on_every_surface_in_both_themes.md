...
| 023+ | The remaining rebuild stories, cut from 018's "What we lack": the task page (thread rows, result file card and preview pane, Work Area panel with Deliverables, credits notice), Assets (From you / Star tabs, chip empty states, the tile ⋯ menu), and the pages behind the sidebar (Plugins and Manage, Scheduled, Connect mobile, MaxHermes, MaxClaw) rendered as the reference renders them and inert | To draft after 022 |
