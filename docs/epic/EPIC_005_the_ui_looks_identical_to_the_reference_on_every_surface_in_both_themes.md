...
| 019 | [The UI has the reference's dark mode, and every control either works or says why not](../story/STORY_019_the_ui_has_the_references_dark_mode_and_every_control_either_works_or_says_why_not.md) — the dark token set and the inert-control notice, the two things the owner named | Drafted (2026-09-14); its UI Mockup can now cite 018's capture — the next story to approve |
| 021+ | One rebuild story per surface group, drafted from 018's inventory (its "What we lack" section): the shell (sidebar sections, Recents rows and menu, Inbox, user menu, Settings modal, promo card, collapsed rail, narrow drawer), the home (attach menu and submenus, More chip menu, the other modes' Showcase), the task page (thread rows, result file card and preview pane, Work Area panel with Deliverables, credits notice), Assets (From you / Star tabs, chip empty states, the tile ⋯ menu), and the pages behind every sidebar item (Search dialog, Plugins and Manage, Scheduled, Connect mobile, MaxHermes, MaxClaw, Create project dialog) rendered as the reference renders them and inert. **Numbered from 021: STORY_020 was taken by EPIC_004's extension work on 2026-09-14** | To draft after 019 |
