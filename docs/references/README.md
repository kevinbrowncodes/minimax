...
| `examples/PR15375_droz_MiniMaxH3_BasicMaskedExtension_v1.4_as-run-2026-09-14.api.json` | The same workflow in the API form ComfyUI's own frontend produced for STORY_020's control run on the Spark (CHORE_007), with the three deviations the story records: our `int8_convrot` unet and `nvfp4_awq` text encoder, the SageAttention/chunked-feed-forward patches bypassed, the unshipped `example.png` reference disconnected | this repo, 2026-09-14 |
