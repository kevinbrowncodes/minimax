...
- [x] **A budget the owner set** before the GPU runs — overnight (13:00); the story stops at it and reports what it has.
...
### Done note, part 1b — night 2, the sample-size pass (2026-09-21 07:15 EDT)

The owner (2026-09-20 12:25): "please create clips until 8am tomorrow that way when we rate them we should have a good sample size"; 15:20: "I really care about penis and buttocks tbh"; 15:35 (by the tool): 4 solo + 3 buttocks per condition. A third prompt, **P3 buttocks** (written with the same skill: he plants a hand on the chair's arm, pushes up out of the recline, kneels facing the chair's back and looks back over his shoulder), three new seeds (424242, 8675309, 31337). **45 of 45 draws**, 14:52 EDT 09-20 → 07:05 EDT 09-21, the SFW container back at 07:08 inside the 07:30 gate: the solo prompt on two seeds under all nine stacks, the buttocks prompt on three. **The matrix stands at 83 of 83 draws** — 16 at rest, 37 solo, 27 buttocks and the 30 s chain's three segments; per stack 2 at rest, 4 solo, 3 buttocks (C6 has no at-rest row by design; C1 a fifth solo, the third-seed row).

| Stack | Solo min | Buttocks min | Peak GiB | Buttocks flag at | Two flags |
|---|---|---|---|---|---|
| C0 | 19.2–19.3 | 19.0–19.1 | 67.8–69.1 | 0.8–1.1 s | 0 |
| C1 | 19.2–19.3 | 19.0–19.1 | 68.4–69.1 | 0.9–1.5 s | 0 |
| C2v1 | 19.1–19.2 | 19.0–19.1 | 68.3–69.0 | 0.9–1.7 s | 0 |
| C3 | 19.0–19.2 | 19.1 | 68.5–68.9 | 1.1–2.2 s | 0 |
| C4 | 19.1–19.2 | 19.1 | 68.8–69.3 | 0.9–1.5 s | 0 |
| C5 | 27.9–28.2 | 27.8–28.0 | 68.2–70.2 | 0.5–0.9 s | 0 |
| C6 | 19.1–19.2 | 19.0–19.1 | 68.6–69.0 | 1.1–1.5 s | 1 |
| C7 | 8.5 | 8.4–8.5 | 55.9–56.8 | 0.9–1.0 s | 1 |
| C8 | 19.0–19.1 | 18.9–19.0 | 56.1–57.0 | 0.9–2.2 s | 0 |

The costs repeat night 1 to the tenth of a minute. No solo draw flagged a cut on any seed (44 of 44 across both nights). Every buttocks draw flagged one framing change at 0.5–2.2 s — the push-up the prompt asks for, read by STORY_060's classifier as a framing change, so the flag is the action; two draws on seed 31337 (C6, C7) flagged a second at 3.5 s, the turn — whether those cut is a question for the rating. NaughtyTimes starts the action earliest on every seed.

**The Spark:** the software limiter on most of every draw (GPU 83–84 °C peak), the hottest zone ≤ 92 °C, the guard's pauses ≈ 80 min in all (< 2 min per draw). **A correction to part 1's "no hardware slowdown":** the throttle bitmask showed the hardware-slowdown bits on single one-minute readings — one in night 1, one in the morning, six in night 2 — with the SM clock at 2418–2450 MHz each time: momentary flags, never a sustained slowdown. The full table and the flag times are in `test/26-09-19-1800_nsfw/run.md` › Night 2, summed up.

**The judgement (part 2) is the owner's:** every one of the 83 clips (the 80 five-second draws and the three film segments) is on [the rating page](https://claude.ai/artifact/UHvx72tfFnGDCYXr7xAnji) — six frames, the run's numbers, a Download button, the questions per prompt, a 1–5 score, free text per clip and for the set; the answers save to the page's store and are read from there. The answer and STORY_064's template follow it.

### Done note, part 2 — the owner's judgement (2026-09-22 15:20 EDT)

**All 83 clips scored**, 2026-09-22 11:08 → 15:14 EDT, on [the rating page](https://claude.ai/artifact/UHvx72tfFnGDCYXr7xAnji); the answers read back from the page's own store and committed verbatim as **[SPIKE_001_ratings.csv](SPIKE_001_ratings.csv)** (83 clip rows — score, every per-prompt question, the owner's free text — plus the set-level row), so the judgement survives the Artifact. Seven clips carry a note. The set-level answer to *which matters more to you here*: **both** (penis and buttocks equally).

| Lever | C7 | C4 | C1 | C5 | C0 | C8 | C2v1 | C3 | C6 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| mean | 4.00 | 3.89 | 3.70 | 3.22 | 3.22 | 3.00 | 3.00 | 2.11 | 1.57 |
| clean | 7/9 | 4/9 | 6/10 | 5/9 | 4/9 | 2/9 | 2/9 | 2/9 | 0/7 |
| stroke | 4/4 | 3/4 | 2/5 | 0/4 | 0/4 | 1/4 | 0/4 | 0/4 | 0/4 |
| at rest | 4.00 | 4.00 | 5.00 | 5.00 | 5.00 | 5.00 | 3.50 | 4.00 | — |
| solo | 4.75 | 4.00 | 3.40 | 1.75 | 2.00 | 3.00 | 2.75 | 1.50 | 1.25 |
| buttocks | 3.00 | 3.67 | 3.33 | 4.00 | 3.67 | 1.67 | 3.00 | 1.67 | 2.00 |
| minutes | 8.5 | 19 | 19 | 28 | 19 | 19 | 19 | 19 | 19 |
| peak | 56 | 69 | 69 | 69 | 69 | 57 | 68 | 69 | 69 |

Mean of the 1–5 score; **clean** = scored 4 or 5 with no anatomy answer below "looks right"; **stroke** = *the penis during the stroke* answered "looks right" over the draws that asked it (the question the matrix was built for); the three prompt rows are mean score; minutes and peak GiB from part 1/1b.

**What the ratings settle.** **C7 — Eros Max beta5 TURBO-hybrid int8 at 8 steps + HMPenis v1.0 @0.5 — leads on every measure that matters here**: the highest mean, the most clean clips, the best solo mean (4.75), and the only stack that held the penis through the stroke on all four draws. It is also the cheapest thing in the matrix (8.5 min against 19, 56 GiB against 68). C4 (Mystic + diogod's Male Anatomy) is second at 3.89 and the best of the Mystic family. **At rest is solved by the stock model** — C0, C1, C5 and C8 all average 5.00 with no adapter needed. **Buttocks belong to C5** (NaughtyTimes, 4.00, and the only stack answering "looks right" on all three *between them* rows); C7 is mid at 3.00 and C8 worst at 1.67. **Two stacks are dead ends:** C6 (Mystic + Penis Lora + HMNSFW AIO) is last at 1.57 with nothing clean and likeness broken on 3 of 7 — the third adapter degraded it below the stock model — and C3 (Mystic + Penis Lora) is 2.11.

**Where the night's hypothesis was wrong.** § Findings 4b predicted "stock FL2VA + Mystic XXX + HMPenis **or** diogod's Male Anatomy … with the act LoRA the prompt needs on top". Half of that held: C4 was one of the two named favourites and came second. The rest did not. (a) § Findings 4 filed Eros Max as "convenience and speed, **not a different model**" and "female-biased by its own users' account" — it won, on the stroke question above all; the desk half was right about the speed and wrong about the model. (b) HMPenis and diogod's were treated as interchangeable; they scored 3.00 and 3.89. (c) "the act LoRA … on top" was the recommended enhancement and produced the worst stack in the set. (d) Penis Lora, "the broadest male-genital adapter by its own caption list", came second from last. What the desk half got right besides C4: **the wobble is the real problem and the field has not solved it** — every stack but C7 was 0–3 of 4 on the stroke, stock and NaughtyTimes at 0.

**Caveats this note records, so the answer is not read as more than it is.**

- **C2v1 is not the condition the matrix specified.** It ran HMPenis **v1.0 @1.0**, not v2 @0.9 (v2 is Civitai-only — part 1's correction), so HMPenis v2 is untested. C7 also carries v1.0 (@0.5) and won, which weakens but does not close the objection.
- **The 30 s film was drawn on C3**, the second-worst stack, before any rating existed. Its *engineering* held — both seams PASS, 753 frames, one shot, no cut — but its anatomy scored 1 / 2 / 1 with hand and penis-in-motion wrong throughout and both joins visible to the owner. **No conclusion about extensions under the winning stack can be drawn from it**, and the matrix's row 9 (one +10 s extension on each of the top two conditions) is still not drawn — which is why that acceptance criterion stays open.
- **The scores are one viewer's, one pass, on six-frame strips plus playback**, 2 at rest / 4 solo / 3 buttocks per stack. Gaps under half a point (C4 vs C1, C0 vs C5, C8 vs C2v1) are inside that noise; the C7 lead on the stroke row is not.
