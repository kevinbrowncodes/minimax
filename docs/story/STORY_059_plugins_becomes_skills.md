...
**Status:** Done (2026-09-18 14:25 EDT — built and gated; deployed with STORY_057 and 058 once the Spark's draw had finished, never mid-job; the screenshots are the addendum. Approved 13:45 "lets proceed with story 59"; drafted 13:15 before any code)
**Created:** 2026-09-18
...
- [x] **`/skills` is the page**: the heading *Skills*, the search, the Director skills and Templates sections exactly as STORY_054 built them, no tabs, no counts; `/skills?create=1` opens the Create template form. `/plugins`, `/plugins?tab=…` and `/plugins/manage` redirect to `/skills` (the links people may have kept keep working); the chip's *Manage skills* and `+ › Skills › Manage skills` / *Add template* go to `/skills` and `/skills?create=1`.
- [x] **The sidebar row reads Skills** with the skill glyph, in the rail and in the full sidebar, current on `/skills`; the *You can now find Agents in Plugins* guide card and its preference are removed.
- [x] **The Plugins, Apps and Agents tabs are removed** with their code (`reference-pages.ts › MANAGE_TABS`, `AGENTS`, `AGENT_SYSTEM_PROMPT`, the Agents editor's markup, `/plugins/manage`); the *video-creator* row and its switch go with them.
- [x] **`videoEnabled` is retired**: dropped from `Settings`, the parser (an old `settings.json` with the key is read without it), the PATCH route (the key answers 400 like any unknown one) and the composer's gate; STORY_058's *Video generation is off* placeholder goes with it — the home has one state.
- [x] **The plugin's traces go**: the composer's `● video-creator` tag (the line starts at the box), the task page's `@video-creator` (the bubble starts at the prompt), and the `MiniMax-M3` pill and its inert menu (`AGENT_MODELS`, the Thinking switch) — the right-hand pill shows **only while the Agent chip is on**, naming the director's model as STORY_050 built it.
- [x] **Both widths, both themes**; the specs that named the tabs, the switch, the guide card, the tag, the mention or the pill are updated and say so.
...
1. **`lib/reference-pages.ts` went whole** — `MANAGE_TABS`, `AGENTS` and `AGENT_SYSTEM_PROMPT` were its only exports and the Management page their only reader; its test with it. `lib/route-title.ts`'s page table now names `/skills` (the active row and the top bar's `page` kind), and `/plugins` reads as *other* — it redirects before it renders.
2. **The Skills page has no counts** — the AC said "no tabs, no counts"; STORY_054's e2e still asserted the tab's number and was the one failure of the first gate run. It now asserts the page's heading.
3. **The plugin-off state of STORY_058 is gone with the setting** (its placeholder, the disabled Send, the director rows' reason): the composer's `video` flag is always true — kept as the name the controls read, so the diff stays small; `canSend` is unchanged. `Composer.test`'s two plugin-off cases became the one-mode cases; the STORY_040 e2e case (the switch) became `skills.spec`'s redirect-and-page case.
4. **An old `settings.json` with `videoEnabled`** parses to the defaults for that key (dropped, not carried): `parseSettings` never copies unknown keys, so nothing was needed beyond removing it from the type and the tables; a PATCH with the key answers 400 *videoEnabled is not a setting*, like any unknown key.
5. **The `Sidebar` lost `onDismissGuide`** and `shell-prefs` its `guideDismissed` with the card; a stored preference file that still has the key is read without it, as with `promoDismissed` in STORY_058.
6. **`AgentModelMenu` takes its model as a required prop** — the reference's cloud entries and the Thinking switch were its only other branch; the menu is STORY_050's one-row menu, and the pill that opens it renders only while the Agent chip is on.

## Done note (2026-09-18)

**Built** (13:50 → 14:15 EDT): `components/pages/SkillsPage.tsx` (the Skills tab's block of `ManagePage.tsx`, lifted; the heading *Skills*, the search, the two sections; `ManagePage.tsx` deleted), `app/skills/page.tsx`, `/plugins` and `/plugins/manage` redirecting (with `?create=1` kept), `Sidebar.tsx` (the row *Skills* with the skill glyph in the rail and the full sidebar; the guide card and its preference gone), `lib/route-title.ts` (`/skills`), `lib/settings.ts` + the PATCH route (`videoEnabled` retired), `Composer.tsx` (the tag gone; the pill only with the chip on; the links to `/skills`; the plugin-off state gone), `ComposerMenus.tsx` (the one-row menu), `TaskPage.tsx` (no `@video-creator`). Unit: `settings-store.test`, `shell-prefs.test`, `route-title.test`. Component: `SkillsPage.test` (renamed from `pages.test`: the page's shape, the director rows, the templates, the create form), `Sidebar.test` (the row, no guide), `Composer.test` and `AgentChip.test` (no tag, no pill with the chip off; *Gemini 3.8 Flash* with it on). Integration: `settings.test` (`videoEnabled` → 400 with its message). E2E: `manage.spec` → `skills.spec` (the redirects, the page without tabs or the switch, the template created / listed / used), `shell.spec` (the Skills row current there; no guide card; three redirects; no Management heading), `agent.spec` (the tab count → the heading), `composer.spec` and `scheduled.spec` (no tag, no pill), the trial spec's link. **Gate** 6/6 by hand (e2e 139). **Deployed** after the Spark's draw with STORY_057 and 058; the screenshots beside `page-plugins@1440` are the addendum below.

**Addendum — verified on the deployed app (2026-09-18 15:05 EDT; `app/e2e-trial/home.spec.ts`, the screenshots `spark/data/smoke/home-*` and `skills-page-light-1440-2026-09-18T19-04-52-070Z.png`):** the sidebar reads *New task · Search · Skills · Scheduled · Assets*, no guide card; *Skills* opens `/skills` — the heading, the search, the two sections, no tabs; `/plugins` answers 307 → `/skills`; `GET /api/settings` no longer carries `videoEnabled`; the composer's line starts at the box (no tag), the bar's right side holds *Run at…* and Send alone with the chip off. Beside `page-plugins@1440`: the page keeps the row anatomy of the Skills tab (STORY_054's side-by-side) under a one-word heading in the reference's heading type; the tabs and the Plugins row are the Departure.
