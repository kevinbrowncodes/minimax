...
**Manual verification on the Spark (2026-09-17 13:02 → 13:55 EDT; MiniMax-H3 `minimax_h3_fl2va_int8_convrot`, text encoder `nvfp4_awq`, ComfyUI 0.35.1, adapter 1.5.0; the director `gemini-3.8-flash` on global at thinking LOW, two passes, 15 s):** job `20ac5d43`, the office photo, notes "keep the camera still", 16:9 768P 10 s — **done in 52.6 min: 243 frames, 10.125 s, `cuts: []`, `camera: static`**. The six-frame strip (`spark/data/smoke/agent-chip-draw-20ac5d43-strip.png`, every 40th frame): the set identical in every frame — the workshop, the black mesh chair, the desk with its papers, the bagged textiles, the fluorescent light — the framing static, the man's left hand rising to the tie knot through the middle frames as the prompt wrote it, and the hold at the end. **The first Gemini-written prompt to run on the model held its set** — the same outcome as the hand-written office round (4 of 4 held, 2026-09-16), at LOW. The reply text: `spark/data/smoke/agent-chip-reply-2026-09-17T17-02-31-753Z.txt` (545 words). The trial (`app/e2e-trial/agent-chip.spec.ts`) passed in 52.9 min.
