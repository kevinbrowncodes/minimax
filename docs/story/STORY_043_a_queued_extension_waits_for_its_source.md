...
**Addendum, 2026-09-16 05:00 EDT (CLAUDE.md § 3.8 — a claim about live state corrected where it was made):** "no hand on the keyboard" was true, but a script's hand was on the API — the verification's waiter polled `GET /api/jobs/:id` for the source every 30 s, and that poll is what wrote the source's `done` into history for the runner to see. The runner never asks the adapter itself, so with every tab closed and no poller the extension waits until someone opens the UI: the owner's overnight run of 2026-09-15/16 idled the GPU 5 h 21 min on exactly this — [BUG_009](../bug/BUG_009_a_queued_extension_waits_for_its_source_only_while_a_browser_is_watching_it.md), found by the peer session. The first AC's "the moment the source is done (the next tick or poll)" holds only for the poll half until BUG_009 lands.
