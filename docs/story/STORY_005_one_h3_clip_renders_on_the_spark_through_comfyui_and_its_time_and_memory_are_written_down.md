...
| Idle cost / stop | ComfyUI container idle after the run: ~4 GiB used (models released by `--cache-none`). `stop.sh` first went SIGTERM → 30 s → SIGKILL (exit 137) because ComfyUI ignores SIGTERM; the compose service now sends **SIGINT**, on which ComfyUI logs `Stopped server` and exits 0 in ~2 s (measured after the run) |
