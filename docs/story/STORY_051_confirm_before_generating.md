...
**Verified live (2026-09-17 13:59 → 14:53 EDT, the app deployed once STORY_050's draw had finished — never mid-job; driven through the real UI by `app/e2e-trial/agent-chip.spec.ts` with `TRIAL_CONFIRM=never`, the screenshots in `spark/data/smoke/agent-chip-settings-*`, `agent-chip-straight-through-task-*`):** ⚙ → the panel with *Always* checked and *Gemini 3.8 Flash · Vertex AI* → *Never* → Save (`/api/settings` → `agentConfirm: "never"`); the office photo, "keep the camera still", Send → *Thinking…* → the director answered in 70 s (Google's latency that minute; two passes, no findings) → **no review step: job `ae4c3ff4` accepted at +71 s and the task page opened** — the box never showed the prompt; the setting put back to *Always* by the trial. **The draw (MiniMax-H3 `int8_convrot`, ComfyUI 0.35.1, adapter 1.5.0): done in 52.3 min — 243 frames, 10.125 s, `cuts: []`, `camera: static`**; the six-frame strip (`spark/data/smoke/agent-straight-draw-ae4c3ff4-strip.png`) shows the set identical in every frame and the same tie beat as STORY_050's draw. Two Gemini prompts, two held sets, at thinking LOW.
