...
- **The owner's full three-script chain the same day** (`2f980101` → `7f201441` → `98eb33ca`, 51 + 67 + 71 min, 753 frames = 31.375 s, peaks 71 / 87 / 87 GiB): both extension seams continuous (0.08 and 0.04 of the clip's largest natural change) — **but** the base model cut on its own inside the fresh segment 1 (frame 142, wide → close-up), and segment 2 held the set for 1.4 s past the overlap and then dissolved to a different set (frames 270–278). The morning run did neither. The mechanism does what this story claims — the seam is the same shot — and the model can still decide to cut seconds later, seed-dependent; that is recorded as [BACKLOG_004](../backlog/BACKLOG_004_an_extension_can_still_change_the_scene_on_its_own_after_the_overlap.md) with the mitigations to measure, rather than folded into this story's Done.
