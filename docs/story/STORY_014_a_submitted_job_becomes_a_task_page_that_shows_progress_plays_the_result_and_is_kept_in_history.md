...
- [ ] `POST /api/jobs` (STORY_009's route) records the entry **before** answering the browser, so a job can never exist without a history row; the task page and Recents read from the store, the job's live status from `GET /api/jobs/:id`.
- [ ] `/task/[id]` renders the thread: the user bubble (`@video-creator` + prompt, reference thumbnails when any), the working indicator with the state text ("Queued…", "Generating… N %"), the Progress panel with the four fixed steps ticked as the job advances, the docked composer, the footer line. `pollUntilTerminal` drives it (1/2/5 s), and every status response is `PATCH`ed into history so a reopened page shows the last known state instantly.
- [ ] **Stop generation**: while the job is queued or running, the docked composer's Send is the black square Stop; clicking it `DELETE`s `/api/jobs/:id` and the page shows "Cancelled at N %"; the cancelled job stays in history as cancelled ([CLAUDE.md → §6 rule 4](../../CLAUDE.md#6-key-rules)).
- [ ] **done**: the result card renders a `<video controls playsinline>` with `src=/api/jobs/:id/result`, `poster=/api/jobs/:id/poster`, the summary line (duration, size, resolution), a **Download** link (`download="<title>.mp4"`) and **Copy prompt**; playback starts on the user's click, never automatically.
- [ ] **failed** shows "ⓘ Request failed — <message>" with a **Retry** pill that resubmits the same request as a new job; `error.code: moderated` shows "The prompt was refused on content grounds" without Retry; `unreachable` (STORY_009's polling) shows "The Spark stopped answering" with Retry.
- [ ] Reopening `/task/:id` for a finished job shows the result without polling; for an open job polling resumes; the entry's `openedAt` is set so the Recents dot clears.
- [ ] The docked composer can start a new job (same Send path as STORY_013, navigates to the new task) once the current one is terminal; while a job runs it only offers Stop.
- [ ] Narrow (390): Progress panel below the thread, video fills the width, controls ≥ 44 px.
