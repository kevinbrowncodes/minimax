...
**Correction (2026-09-12, found during STORY_006):** the `run.sh` shipped by this story had a bug — `run_step` ended with `if [ "$1" = "build" ]`, and a shell function returns the status of its last statement, which for every non-build step was that `if` (0). Every non-build step's failure was therefore reported as "ok" between this story's push and the fix in STORY_006's commit; `run.test.sh` did not catch it because its dry-run seam was a separate branch that returned early. Re-running the true gate after the fix showed that the only failures masked in that window were in STORY_006's own new package (two wrong test expectations, 19 lint errors), all fixed before its commit; the earlier stories' lanes were clean. The fix captures each step's status explicitly (`|| rc=$?`) and returns it, and the dry-run path now goes through the same status logic. Lesson recorded in the STORY_006 Done note: put a test seam at the command executor, never as a parallel branch.
