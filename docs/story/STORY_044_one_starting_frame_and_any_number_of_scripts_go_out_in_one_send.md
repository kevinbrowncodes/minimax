...
**Manual verification on the Spark** (model MiniMax-H3 FL2VA `int8_convrot`, text encoder `nvfp4_awq`, ComfyUI 0.35.1, adapter 1.4.0, app deployed 11:46 EDT): the story's (a), driven through the real UI by `app/e2e-trial/send-all-chain.spec.ts` (not in the gate) — `01.jpg` + a scene and three 5 s scripts (`spark/data/input/chain5s/text.txt`), Send all at 11:48 EDT: the strip read "3 segments · 5 s each · ≈ 16.5 s in all · overlap 1.6 s", the three 202s came in order (`8a10fa72 → 703c9d92 → de40d4a5`, positions –, 1, 2), the last segment's page opened reading Queued, Scheduled showed 1 Running and 2 Waiting. The segments' outcomes (the adapter's log, UTC−4):

| Segment | App id → adapter job | Generated | Result |
| --- | --- | --- | --- |
| 1 — 01.jpg + script 1, 5 s | `8a10fa72` | 11:48:49 → 12:07:56 (19 min) | 124 frames, 5.2 s, `cuts: []` |
| 2 — +5 s, script 2 | `703c9d92` → `99028459` | **submitted 12:08:00**, 4 s after segment 1's done, by the runner with no page polling the source (BUG_009's fix under a chain) → 12:37:53 (30 min) | 260 frames, 10.8 s, `cuts: []` |
| 3 — +5 s, script 3 | `de40d4a5` → `fc152b5b` | **submitted 12:37:58**, 5 s after segment 2's done → 13:07:57 (30 min) | **396 frames, 16.5 s** — the strip's "≈ 16.5 s in all" to the frame, `cuts: []` |

1 h 19 min unattended for the three; the trial's own checks passed (`1 passed (1.3h)`): the last segment's page holds the whole video, "Continues" naming segment 2. Frames viewed 0 / 60 / 122 / 124 / 180 / 256 / 260 / 320 / 395: one shot — the sequin curtain, both lamps and the stanchions unchanged throughout, both seams invisible, the poses in the scripts' order (palms to thighs, chest out, hands to hips, the three-quarter pivot, the side chest, arms settling at the sides). Note for the README's cost table: a +5 s extension took 30 min here against 19 min for the fresh 5 s clip — the extension generates 175 frames (5 s + the 1.6 s overlap) and rides the source through every step.

**Not run:** the story's (b) — the owner's own 10 s chain (`01.jpg` + `scene.txt` + `script1–3`) as one Send, ≈ 3 h 5 min of GPU; the mechanism is the same three POSTs, proven above at 5 s, and the owner runs it when he next wants that video.
