...
**Addendum — the Spark's x2 draw (2026-09-17 22:50 → 2026-09-18 00:32 EDT; `gemini-3.8-flash` at thinking LOW; MiniMax-H3 FL2VA `int8_convrot`, ComfyUI 0.35.1, adapter 1.5.0):** through the real UI (`app/e2e-trial/agent-draws.spec.ts`, the log `spark/data/smoke/agent-draws-trial-20260918T025017.log`, the screenshots `agent-draws-settings-*`, `-reply-*`, `-queued-2026-09-18T02-50-29-338Z.png`): the chip on *Thirst trap* → ⚙ → **x2** → Save (`agentDraws: 2` answered by the PATCH) → the office photo, "keep the camera still" → *Send message, 2 draws* → the director answered in 15 s (2 passes, 0 findings; the reply in `agent-draws-reply-2026-09-18T02-50-29-338Z.txt`) → the box with the prompt, Send reading **×2**, the line **≈ 2 × 50 min on the Spark** → Send → **two 202s: 93defa58, 9b7f13e6** (the same prompt, the photo on each, no `seed` in either request), the first draw's task page; the setting put back to x1. **What the line looked like:** the adapter accepted both at once and holds its own queue, so the second draw was *queued* at the adapter (not in the app's line: no ordinal in the toast, *Queued — 2 draws*) and started the moment the first finished — the "one Running, one Waiting" of the plan is the adapter's queue, not STORY_041's. **The seeds, from the adapter's log** (`docker logs minimax-adapter`, the *submitted as ComfyUI prompt* lines): **3888127104** for 93defa58 and **618256694** for 9b7f13e6 — the adapter's own draws, different, as the AC promised. The draws, followed by `agent-chain-poll.sh` on the ids json: draw 1 done at 23:43 (51 min; 243 frames, cuts `[]`, camera *moving*), draw 2 at 00:32 (49 min; 243 frames, one framing flag at frame 36 / 1.5 s, camera *moving*). **The strips** (`agent-draws-1-93defa58-strip.png`, `agent-draws-2-9b7f13e6-strip.png`): the set identical in both from the first frame to the last, the same beats in each — the left hand to the tie knot through the middle seconds, back to the armrest, the hold to the lens — and the two differ the way two draws should: draw 1 keeps the phone's framing steadier and settles into a lower, closer hold; draw 2 reframes wider in its first second and a half (the flag) and holds with the arm along the armrest. **The better draw: 93defa58** (the steadier frame, no framing flag). Both share the title *A young man in his early twenties with thick, neatly…* in Recents (the Departure as written; CHORE_015 would give them their beat instead). Straight-through with draws (AC 2's third case) was proved in the e2e; on the Spark the *Never* run of the night was the chain (STORY_053's second addendum), which the draw count does not touch.
