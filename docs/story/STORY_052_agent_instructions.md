...
**Addendum — the Google-only manual verification (2026-09-17 19:00 → 19:10 EDT, on the deployed app, ≈ 4 ¢):** two instructions saved through `PUT /api/agent/instructions` — *Studio · sequin curtain* (the anchor-example text, 259 words) with the owner's studio photo `spark/data/input/01.jpg` uploaded as its reference through `POST …/:id/reference` (200; served back as `image/jpeg`), and *House rule* — *"The camera stays completely fixed unless the script moves it. Never add a camera move the notes did not ask for. The overall_soundscape always ends with this exact sentence: A wall clock ticks once."* — the clock sentence being the discriminator: it is in no skill file and no reference, so it can only reach the reply through the instruction. The office photo through `POST /api/agent/runs` twice: with the notes "keep the camera still" — 200 in 13.5 s, two passes, 591 words, no findings, the camera sentence *"holds a perfectly static shot throughout the entire ten-second duration"*; and **with no notes at all** — 200 in 14.3 s, two passes, 574 words, no findings, the same camera sentence, and the soundscape ending *"…rustles gently against the mesh chair as weight subtly shifts. A wall clock ticks once."* — the rule arrived, exactly once, where it was asked to go. The studio instruction's set did **not** leak into the office scene (no *sequin*, no *curtain* in either reply): the labelled example image with its paragraph is read as an example of the format, as intended, not as the scene. Through the real UI (`app/e2e-trial/agent-instructions.spec.ts`, new — the story's driver, never in the gate; `spark/data/smoke/agent-instructions-badge-*`, `-panel-*`): the chip on, the ≡ labelled *Agent instructions, 2 active* with the badge **2**, the panel with both rows — the switches on, the titles, the texts, the studio tile rendered from its own bytes (`naturalWidth > 0`) — Escape, the store unchanged. The two instructions stay saved on the box for the chain trial (STORY_053).
