...
**Addendum — verified on the Spark (2026-09-18 12:48 → 15:03 EDT; `app/e2e-trial/retry-chain.spec.ts`, the log `spark/data/smoke/retry-chain-trial-*`, the screenshots `retry-chain-notice-*`, `retry-chain-redraw-2026-09-18T16-47-43-731Z.png`):** on the page of last night's cut segment `7b2636b9` (its `chainAfter`: `c09bc6fa`, done) the notice read *The shot changed at 00:10 … Retry redraws this segment and the 1 segment queued after it with new seeds.* with **Retry chain** → one click → `POST /api/jobs/7b2636b9…/retry-chain` answered 202 in under a second with the redraw `f4abcfc7` and the re-chained `ffb3ffcd`; the toast *Redrawing this segment and 1 after it*; the redraw's page; `ffb3ffcd` in history continuing from `f4abcfc7`, `f4abcfc7` from `eb90ccb2` (segment 1) — the old `c09bc6fa` left as it was (done). **The mechanism did exactly what the story says.** The draws (`retry-chain-2026-09-18T16-47-43-731Z.log`; seeds from the adapter's log): the redraw done at 13:59 (67 min; 498 frames) — **and it cut at its join again** (`cuts: [{ frame: 243, kind: "cut" }, { frame: 312, kind: "framing" }]`, the seam measure FAIL at 12.78, a new frontal desk-level shot as the man leans — `retry-chain-seg2-f4abcfc7-strip.png`); the re-chained segment 3 done at 15:03 (64 min; 753 frames) and it **cut at its own join too** (`{ frame: 498, kind: "cut" }`, mean diff 80.4 — `retry-chain-seg3-ffb3ffcd-strip.png`). So this prompt has now been drawn four times at its segment 2 (three joins) plus two segment 3s: **four cuts in six joins**. The feature recovers a chain in one click; the model's odds on this beat are what STORY_060 is for — the owner's rule of the day: Retry chain is a safety net he never wants to need.
