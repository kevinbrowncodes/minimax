...
- Same rule on start (CHORE_003, same day): `recover()` consults ComfyUI's queue; an open job ComfyUI still lists is kept open instead of being failed as "adapter restarted while the job was open", so the adapter can be redeployed under a running job. Test: `server.test.ts` "keeps a job open across an adapter restart while ComfyUI still lists it".
